# cached_execution(cache, proc, proc_input) takes in
...
def cached_execution(cache, proc, proc_input):
    # format of cache {proc_input1: proc_output1, proc_input2: proc_output2, proc_input3: proc_output3,...}
    if proc_input not in cache:                     #look if the input is in the cache
        cache[proc_input] = proc(proc_input)        #if the input is not in the cache, you add it to it
    return cache[proc_input]                        #for the input, return the value of the cache
